...
MatchingPath = os.path.join(WorkingDirectory, '04_Results/02_Individuals_Matching/')



# 03 Binarized resized ROI and write file
Resized_ROIs = [File for File in os.listdir(ResizedROIsPath) if File.endswith('.mhd')]
MatchList = pd.read_csv(MatchingPath+'Matched_'+Group+'.csv')


for Resized_ROI in Resized_ROIs:

    if Group == 'Healthy':
        Test = Resized_ROI[6:10] in list(MatchList['Sample Number'].astype('str'))

    elif Group == 'OI':
        Condition1 = Resized_ROI[2:8] in list(MatchList['Patient_ID'].astype('str'))
        Condition2 = Resized_ROI[9:17] in list(MatchList['Scan_ID_BSL'].astype('str'))
        Test = Condition1 & Condition2

    if Test:

        # Load ROI
        File = ResizedROIsPath + Resized_ROI
        CT_Scan, Origin, Spacing, Size = Load_Itk(File)

        # Change values to binary 1 and 2
        Threshold = 50
        ...
        OldName = Resized_ROI[:-4]
        MHDText = MHDText.replace(OldName, Resized_ROI[:-11] + 'Binarized')
...
PipelineFile  = open(os.path.join(WorkingDirectory, '03_Scripts', '03_ROIMorphometry.txt'), 'r')
