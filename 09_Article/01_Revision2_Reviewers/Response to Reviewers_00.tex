...
\begin{quote}
	The stiffnesses of highly heterogeneous ROIs were randomly lower with respect to the fabric-elasticity relationships, which reflects the limit of validity of the computational homogenisation methodology. \Add{However, this limitation does not challenge the fabric-elasticity relationship, which extrapolation to heterogeneous ROIs may be reasonable but can simply not be evaluated with the employed homogenisation methodology. Moreover, due to their low BV/TV, the potential (unknown) errors on these heterogeneous ROIs would have little influence on whole bone stiffness in comparison to homogeneous ROIs which are orders of magnitude stiffer.}
\end{quote}

Discussion, page 9, first column, line 56-59
\begin{quote}
	The CV shows that heterogeneity of OI trabecular bone is higher compared to healthy control and is consequently the more discriminant parameter. \Add{This comes from the fact that high heterogeneity is found at the edge of void spaces or medullary cavities that are much larger in OI.}
\end{quote}

Discussion, page 9, second column, line 55-60
\begin{quote}
	Some ROIs with low BV/TV do not have every side of the cube connected by bone, leading to extremely low terms in the stiffness tensor (see Appendix C). Trying to homogenise such ROIs can lead to errors of multiple order of magnitude, as observed on the plot (Figure 5b). \Add{This represents a limitation of the homogenization methodology and t}\Del{T}herefore, a filtering \Add{according to the heterogeneity level} is indispensable to assess and compare fabric-elasticity relationships, as done by Panyasantisuk et al. [28].
\end{quote}
...
\begin{quote}
	Thus, the same hFE methodology can be applied for bone stiffness assessment of OI diagnosed individuals as for healthy ones\Add{, regardless the OI type}. Although in hFE analysis it is not possible to exclude part of the mesh because of high heterogeneity, the influence of such ROIs seems negligible on full bone models as the fabric-elasticity relationships can extrapolate their stiffness smoothly and as this concerns only ROIs with low BV/TV and stiffness\Del{.}\Add{ order of magnitude lower than homogeneous ones and the cortex (< 1MPa compared to 10 GPa). Moreover, studies validated the use of fabric-elasticity relationships in osteoporotic bone [35,17] where medullary cavities are important and therefore lead to locally high heterogeneity.}
\end{quote}
